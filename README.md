...
- `dashboard.html` — Analyse-Dashboard (Coinglass-artig: KPI-Kacheln, Preis-/Volumen-Charts, Adressverteilung, Groß-Transfer-Volumen, Exchange-Netflow, Korrelationsmatrix, Lag-Analyse, Aktivitäts-Heatmap). Liest alle obigen Dateien per `fetch()` (muss über einen lokalen Webserver geöffnet werden, nicht per `file://`).
